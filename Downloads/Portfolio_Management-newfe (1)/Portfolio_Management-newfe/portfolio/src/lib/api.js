...
async function fetchAPI(endpoint, options = {}) {
    ...
    try {
        ...
        // Network / CORS / connection refused etc.
        const msg = err?.message ? String(err.message) : String(err);
        throw new Error(`API request failed to reach ${BASE_URL}/${endpoint}: ${msg}`);
    }
    if (!res.ok) {
        const errorText = await res.text().catch(() => '');
        throw new Error(`API request failed (${BASE_URL}/${endpoint}) with status ${res.status}${errorText ? ': ' + errorText : ''}`);
    }
    return res.json();
}
